...
@section('content')
    ...
    <div class="max-width-1200">
        <div class="flexbox-annotated-section">
            ...
            <div class="flexbox-annotated-section-annotation">
                ...
                <div class="annotated-section-description pd-all-20 p-none-t">
                    <p class="color-note">{{ trans('Setting::setting.email.description') }}</p>
                </div>
            </div>
            ...
            <div class="flexbox-annotated-section-content">
                <div class="wrapper-content pd-all-20" id="email-config-form">
                    <div class="form-group">
                        <label class="text-title-field" for="email_driver">{{ trans('Setting::setting.email.driver') }}</label>
                    ...
                    <div class="form-group">
                        <label class="text-title-field" for="email_port">{{ trans('Setting::setting.email.port') }}</label>
                        <input data-counter="10" type="number" class="next-input" name="email_port" id="email_port"
                               value="{{ setting('email_port', config('mail.port')) }}" placeholder="{{ trans('Setting::setting.email.port_placeholder') }}">
                    </div>
                    <div class="form-group">
                        <label class="text-title-field" for="email_host">{{ trans('Setting::setting.email.host') }}</label>
                        <input data-counter="60" type="text" class="next-input" name="email_host" id="email_host"
                               value="{{ setting('email_host', config('mail.host')) }}" placeholder="{{ trans('Setting::setting.email.host_placeholder') }}">
                    </div>
                    <div class="form-group">
                        <label class="text-title-field" for="email_username">{{ trans('Setting::setting.email.username') }}</label>
                        <input data-counter="60" type="text" class="next-input" name="email_username" id="email_username"
                               value="{{ setting('email_username', config('mail.username')) }}" placeholder="{{ trans('Setting::setting.email.username_placeholder') }}">
                    </div>
                    <div class="form-group setting-mail-password @if (setting('email_driver', config('mail.driver')) == 'mailgun') hidden @endif">
                        <label class="text-title-field" for="email_password">{{ trans('Setting::setting.email.password')  }}</label>
                        <input data-counter="60" type="password" class="next-input" name="email_password" id="email_password"
                               value="{{ setting('email_password', config('mail.password')) }}" placeholder="{{ trans('Setting::setting.email.password_placeholder') }}">
                    </div>
                    <div class="setting-mail-mail-gun @if (setting('email_driver', config('mail.driver')) !== 'mailgun') hidden @endif">
                        <div class="form-group">
                            <label class="text-title-field" for="email_mail_gun_domain">{{ trans('Setting::setting.email.mail_gun_domain') }}</label>
                            <input data-counter="60" type="text" class="next-input" name="email_mail_gun_domain" id="email_mail_gun_domain"
                                   value="{{ setting('email_mail_gun_domain', config('services.mailgun.domain')) }}" placeholder="{{ trans('Setting::setting.email.mail_gun_domain_placeholder') }}">
                        </div>
                        @if (!app()->environment('demo'))
                            <div class="form-group" style="margin-bottom: 1em;">
                                <label class="text-title-field" for="email_mail_gun_secret">{{ trans('Setting::setting.email.mail_gun_secret')  }}</label>
                                <input data-counter="60" type="text" class="next-input" name="email_mail_gun_secret" id="email_mail_gun_secret"
                                       value="{{ setting('email_mail_gun_secret', config('services.mailgun.secret')) }}" placeholder="{{ trans('Setting::setting.email.mail_gun_secret_placeholder') }}">
                            </div>
                        @endif
                    </div>
                    <div class="form-group">
                        <label class="text-title-field" for="email_encryption">{{ trans('Setting::setting.email.encryption') }}</label>
                        <div class="ui-select-wrapper">
                    ...
                    <div class="form-group">
                        <label class="text-title-field" for="email_from_name">{{ trans('Setting::setting.email.sender_name') }}</label>
                        <input data-counter="60" type="text" class="next-input" name="email_from_name" id="email_from_name"
                               value="{{ setting('email_from_name', config('mail.from.name')) }}" placeholder="{{ trans('Setting::setting.email.sender_name_placeholder') }}">
                    </div>

                    <div class="form-group">
                        <label class="text-title-field" for="email_from_address">{{ trans('Setting::setting.email.sender_email') }}</label>
        ...
        <div class="flexbox-annotated-section">
            ...
            <div class="flexbox-annotated-section-annotation">
                <div class="annotated-section-title pd-all-20">
                    <h2>{{ trans('Setting::setting.email.template_title') }}</h2>
                </div>
                <div class="annotated-section-description pd-all-20 p-none-t">
                    <p class="color-note">{{ trans('Setting::setting.email.template_description') }}</p>
                </div>
            </div>
                ...
                <div class="wrapper-content pd-all-20">
                    <div class="table-wrap">
                        <table class="table product-list ws-nm">
                            <thead>
                            <tr>
                                <th class="border-none-b">{{ trans('Setting::setting.template') }}</th>
                                <th class="border-none-b"> {{ trans('Setting::setting.description') }} </th>
                            </tr>
                            </thead>
                            <tbody>
                                <tr>
                                    <td>
                                        <a class="hover-underline a-detail-template"
                                           href="{{ route('setting.email.template.edit', ['type' => 'core', 'name' => 'base', 'template_file' => 'header']) }}">
                                            {{ trans('Setting::setting.email.template_header') }}
                                        </a>
                                    </td>
                                    <td>{{ trans('Setting::setting.email.template_header_description') }}</td>
                                </tr>
                                <tr>
                                    <td>
                                        <a class="hover-underline a-detail-template"
                                           href="{{ route('setting.email.template.edit', ['type' => 'core', 'name' => 'base', 'template_file' => 'footer']) }}">
                                            {{ trans('Setting::setting.email.template_footer') }}
                                        </a>
                                    </td>
                                    <td>{{ trans('Setting::setting.email.template_footer_description') }}</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
        ...
        <div class="flexbox-annotated-section" style="border: none">
            ...
            <div class="flexbox-annotated-section-content">
                <button class="btn btn-secondary" type="button" data-target="#send-test-email-modal" data-toggle="modal">{{ trans('Setting::setting.test_send_mail') }}</button>
    ...
    {!! Form::modalAction('send-test-email-modal', trans('Setting::setting.test_email_modal_title'), 'info', view('Setting::test-email')->render(), 'send-test-email-btn', trans('Setting::setting.send')) !!}
@stop

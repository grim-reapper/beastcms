...
class HookServiceProvider extends ServiceProvider
{
    ...
    {
        if (!empty($data) && in_array(get_class($data), config('Revision.general.supported', []))) {
            ...
            return $tabs . view('Revision::history-tab')->render();
        }
        return $tabs;
    }
    ...
    {
        if (!empty($data) && in_array(get_class($data), config('Revision.general.supported', []))) {
            return $tabs . view('Revision::history-content', ['model' => $data])->render();
        }
        return $tabs;
    }
}

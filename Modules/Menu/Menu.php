...
class Menu
{
    ...
    public function generateMenu($args = [])
    {
        ...
        if (!$this->cache->has($cacheKey) || $this->config->get('Menu.general.cache.enabled') == false) {
    ...
    {
        return $this->config->get('Menu.general.locations', []);
    }
    ...
    public function addMenuLocation(string $location, string $description): self
    {
        ...
        config()->set('Menu.general.locations', $locations);

...
class PluginManagementController extends Controller
{
    ...
    {
        page_title()->setTitle(trans('PluginManagement::plugin.plugins'));
        ...
        return view('PluginManagement::index', compact('list'));
    }
    ...
    public function update(
        ...
        if (!empty($content)) {
            try {
                ...
                if (!in_array($plugin, $activatedPlugins)) {
                    if (!empty(Arr::get($content, 'require'))) {
                        ...
                        if (!$valid) {
                            return $response
                                ->setError()
                                ->setMessage(trans('PluginManagement::plugin.missing_required_plugins', [
                ...
                return $response->setMessage(trans('PluginManagement::plugin.update_plugin_status_success'));
            } catch (Exception $ex) {
                return $response
                    ->setError()
        ...
        return $response
            ->setError()
            ->setMessage(trans('PluginManagement::plugin.invalid_plugin'));
    }
    ...
    public function destroy($plugin, BaseHttpResponse $response, PluginRemoveCommand $pluginRemoveCommand)
    {
        ...
        if (in_array($plugin, scan_folder(plugin_path()))) {
            try {
                ...
                return $response->setMessage(trans('PluginManagement::plugin.remove_plugin_success'));
            } catch (Exception $ex) {
                return $response
                    ->setError()
                    ->setMessage($ex->getMessage());

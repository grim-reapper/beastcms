...
class DashboardController extends BaseController
{
    ...
    public function getDashboard(Request $request)
    {
        ...
        $widgets = $widgets->reject(
            function ($item) use ($availableWidgetIds) {
        ...
        return view('Dashboard::list', compact('widgets', 'userWidgets'));
    }
    ...
    public function postEditWidgetSettingItem(Request $request, BaseHttpResponse $response)
    {
        try {
            ...
            if (!$widget) {
                return $response
                    ->setError()
                    ->setMessage(trans('Dashboard::dashboard.widget_not_exists'));
            }
    ...
    public function postHideWidgets(Request $request, BaseHttpResponse $response)
    {
        ...
        foreach ($widgets as $widget) {
            ...
            if (array_key_exists($widget->name, $request->input('widgets', [])) && $request->input(
                    'widgets.' . $widget->name
                ) == 1) {
        ...
        return $response
            ...
            ->setMessage(trans('Dashboard::dashboard.hide_success'));
    }
}

    ...
    <div class="admin-bar-container">
        <div class="admin-bar-logo">
            <a href="{{ route('dashboard.index') }}" title="Go to dashboard">
                <img src="{{ setting('admin_logo') ? get_image_url(setting('admin_logo')) : url(config('Base.general.logo')) }}" alt="logo"/>
            </a>
        </div>
        ...
        <ul class="admin-navbar-nav admin-navbar-nav-right">
            <li class="admin-bar-dropdown">
                ...
                <ul class="admin-bar-dropdown-menu">
                    <li><a href="{{ route('user.profile.view', Auth::user()->getKey()) }}"><i class="icon-user"></i> {{ trans('Base::layouts.profile') }}</a></li>
                    <li><a href="{{ route('access.logout') }}">{{ trans('Base::layouts.logout') }}</a></li>
                </ul>
            </li>
        </ul>
    </div>
